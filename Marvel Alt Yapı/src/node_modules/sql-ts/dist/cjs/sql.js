...
class Sql {
    ...
    // Define a table
    define(def) {
        def = defaults_1.default(def || {}, {
            sql: this
        });
        return table_1.Table.define(def);
    }
    defineColumn(def) {
        return new column_1.Column(def);
    }
    // Returns a bracketed call creator literal
    array(...args) {
        const arrayCall = new node_1.ArrayCallNode(sliced_1.default(args));
        return arrayCall;
    }
    // Returns a bracketed call creator literal
    row(...args) {
        const rowCall = new node_1.RowCallNode(sliced_1.default(args));
        return rowCall;
    }
    ...
    interval(...args) {
        const interval = new node_1.IntervalNode(sliced_1.default(args));
        return interval;
    }
    // Set the dialect
    ...
    // Create a parameter
    parameter(value) {
        return new node_1.ParameterNode(value);
    }
}
exports.Sql = Sql;
