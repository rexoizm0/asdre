...
class Mssql extends postgres_1.Postgres {
    ...
    visitAlter(alterNode) {
        ...
        const addColumn = () => {
            ...
            for (let i = 1, len = alterNode.nodes.length; i < len; i++) {
                ...
                assert_1.default(node.type === 'ADD COLUMN', errMsg);
        ...
        const dropColumn = () => {
            ...
            for (let i = 1, len = alterNode.nodes.length; i < len; i++) {
                const node = alterNode.nodes[i];
                assert_1.default(node.type === 'DROP COLUMN', errMsg);
    ...
    visitOrderBy(orderByNode) {
        ...
        assert_1.default(offsetNode, 'Something bad happened, should have had an msSQLOffsetNode here.');
    ...
    visitQueryHelper(actions, targets, filters) {
        ...
        const processLimit = (limitInfo) => {
            ...
            assert_1.default(selectInfo !== undefined, 'MS SQL Server requires a SELECT clause when using LIMIT');
