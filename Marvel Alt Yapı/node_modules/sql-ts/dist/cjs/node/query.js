...
class Query extends _1.Node {
    ...
    select(...args) {
        ...
        const flattenedArgs = sliced_1.default(args).reduce((cur, next) => {
    ...
    star() {
        assert_1.default(this.type === 'SUBQUERY', 'star() can only be used on a subQuery');
    ...
    leftJoin(other) {
        assert_1.default(this.type === 'SUBQUERY', 'leftJoin() can only be used on a subQuery');
        return new _1.JoinNode('LEFT', this, other.toNode());
    }
    where(...nodes) {
        if (nodes.length > 1) {
            // allow multiple where clause arguments
            const args = sliced_1.default(nodes);
    ...
    insert(...nodes) {
        let args = sliced_1.default(nodes);
    ...
    forUpdate() {
        assert_1.default(typeof this._select !== 'undefined', 'FOR UPDATE can be used only in a select statement');
        this.add(new _1.ForUpdateNode());
        return this;
    }
    forShare() {
        assert_1.default(typeof this._select !== 'undefined', 'FOR SHARE can be used only in a select statement');
    ...
    drop() {
        if (this.indexesClause) {
            const args = sliced_1.default(arguments);
    ...
    exists() {
        assert_1.default(this.type === 'SUBQUERY', 'exists() can only be used on a subQuery');
    ...
    notExists() {
        assert_1.default(this.type === 'SUBQUERY', 'notExists() can only be used on a subQuery');
        return new _1.PrefixUnaryNode({
            left: this,
            operator: 'NOT EXISTS'
        });
...
extend_1.default(Query.prototype, valueExpressions);
...
extend_1.default(Query.prototype, _1.AliasNode.AliasMixin);
