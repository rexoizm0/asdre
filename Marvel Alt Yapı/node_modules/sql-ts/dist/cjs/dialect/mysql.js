...
class Mysql extends postgres_1.Postgres {
    ...
    visitRenameColumn(renameColumnNode) {
        ...
        assert_1.default(dataType, 'dataType missing for column ' +
    ...
    visitInterval(intervalNode) {
        let parameter;
        if (isNumber_1.default(intervalNode.years)) {
            parameter = isNumber_1.default(intervalNode.months)
                ? `'${intervalNode.years}-${intervalNode.months}' YEAR_MONTH`
                : `${intervalNode.years} YEAR`;
        }
        else if (isNumber_1.default(intervalNode.months)) {
            parameter = `${intervalNode.months} MONTH`;
        }
        else if (isNumber_1.default(intervalNode.days)) {
            parameter = `'${intervalNode.days} ${isNumber_1.default(intervalNode.hours) ? intervalNode.hours : 0}:${isNumber_1.default(intervalNode.minutes) ? intervalNode.minutes : 0}:${isNumber_1.default(intervalNode.seconds) ? intervalNode.seconds : 0}' DAY_SECOND`;
        }
        else {
            parameter = `'${isNumber_1.default(intervalNode.hours) ? intervalNode.hours : 0}:${isNumber_1.default(intervalNode.minutes) ? intervalNode.minutes : 0}:${isNumber_1.default(intervalNode.seconds) ? intervalNode.seconds : 0}' HOUR_SECOND`;
        }
