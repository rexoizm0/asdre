...
class Node {
    ...
    add(node) {
        assert_1.default(node, 'Error while trying to add a non-existant node to a query');
        let newNode;
        if (typeof node === 'string') {
            newNode = new _1.TextNode(node);
        }
        else if (nodeable_1.instanceofINodeable(node)) {
            newNode = node.toNode();
        }
        else {
            newNode = node;
    ...
    unshift(node) {
        assert_1.default(node, 'Error while trying to add a non-existant node to a query');
        this.nodes.unshift(typeof node === 'string' ? new _1.TextNode(node) : node.toNode());
        return this;
    }
    toQuery(dialect) {
...
const determineDialect = (query, dialect) => {
    ...
    if (dialect) {
        ...
        DialectClass = dialect_1.getDialect(dialect);
    }
        ...
        DialectClass = dialect_1.getDialect(dialect_1.DEFAULT_DIALECT);
    }
    return DialectClass;
};
